...
from model_set import Resnet_location_and_predict as Model1_LP
from model_set import Resnet_location as Model1_L
from model_set import ResNet_predict as Model_2
from model_set import ResNet_predict_multi as Model2_multi
...
# =============================================================================
    model1_weights = ['F:/weights/model1_5_3d1_3d2_our_adapt_big.h5',
               'F:/weights/model1_340417_3d1_3d2_our_10x_adapt.h5',
               'F:/weights/model1_340417_3d1_3d2_our_10x_adapt.h5']
    model2_weights = ['F:/weights/model2_670_3d1_3d2_our_10x_adapt.h5'] 
    ...
    model2_type = '2'
    #10x
    sizepatch = {'20x':(int(1936*2),int(1216*2)),'10x':(1936,1216)}['10x']
    sizepatch_small = {'20x_small':(int(512*2),int(512*2)),
                       '10x_small':(512,512),
                       '20x_big':(int(1936*2),int(1216*2)),
                       '10x_big':(1936,1216)}['10x_big']
    widthOverlap = {'20x':(int(120*2),int(120*2)),'10x':(120,120)}['10x']
    #predict
    sizepatch_predict = (1936,1216)#1459 2323
    sizepatch_predict_small = {'small':(512,512),'big':(1936,1216)}['big']
    widthOverlap_predict = {'3*5':(160,156),'4*6':(197,150),'1*1':(1935,1215)}['1*1']
    num = 1
    #20x model2
    ...
    # 建立并行预测模型
    gpu_num = 1
    model1 = Model1_L(input_shape = (1216,1936,3))
    model1.load_weights(model1_weights[0],by_name=True)
    model2 = Model2(input_shape = (256,256,3))
    model2.load_weights(model2_weights[0])#
    ...
    for cz in testfile240:
        ...
        #将拍摄小图拆分成适合网络大小的小图
        start = time.time()
        num,startpointlist_split = Startpointlist_split(sizepatch_predict,sizepatch_predict_small, widthOverlap_predict)
        ...
        for index, item in enumerate(feature):
            if predict1[index] > 0.5:
                _, _, Local_3 = region_proposal(item[...,1], output_shape = sizepatch_predict_small[::-1] ,img=None, threshold=0.2,ratio=0.2)
