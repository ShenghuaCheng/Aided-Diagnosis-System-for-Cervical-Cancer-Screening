...
model1_weights = ['F:/weights/model1_5_3d1_3d2_our_adapt_big.h5',
               ...
               'F:/weights/model1_340417_3d1_3d2_our_10x_adapt.h5']
model2_weights = ['F:/model2_670_3d1_3d2_our_10x_adapt.h5']      
vol_test = 1000
...
for item1 in classes:
    ...
    # model1读图并测试
    imgTotaltest,img_maskTotaltest,local_1 = GetImgMultiprocess(dirList, resize_shape,crop_shape,output_shape,enhance_flag,path_mask, mask_shape,local=None)
    ...
    # 可视化结果
    while(not os.path.exists(pathsave+ item1)):
        os.mkdir(pathsave+ item1)
    for index, item2 in enumerate(imgMTotal):
        img_prop, _,local_2 = region_proposal(item2[...,1], output_shape,imgTotaltest[index], threshold,ratio=0.2)
