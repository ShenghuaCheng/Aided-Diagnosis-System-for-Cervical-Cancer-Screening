...
learning_rate = 1e-4
config_path = r'F:\LiuSibo\Codes\Projects\AidedDiagnosisSystem\doc\manu_yjy\exp2\config'
path_checkpoints = r'F:\LiuSibo\Codes\Projects\AidedDiagnosisSystem\doc\manu_yjy\exp2\weights\stage_AB'

file_pretrain_weights = r'F:\LiuSibo\Codes\Projects\AidedDiagnosisSystem\doc\manu_yjy\exp2\weights\stage_AB\Block_57.h5'
start = 58
end = 800  # 1epoch=20blocks
Frozen_layers = 37

enhance_flag = {'train': True, 'vail': False, 'test': False}
scale_flag = {'train': False, 'vail': False, 'test': False}

DataSet = {x: DataSet(config_path,
                      'stage_AB_valABCD.xlsx',
                      x,
                      crop_num=1,
                      norm_flag=True,
                      ...
                      mask_flag=False) for x in ['train', 'vail', 'test']}
# =====================================================================================================================
# Model Setting
# =====================================================================================================================
...
for block in range(start, end):
    ...
    if block%2 ==0:
        ...
        vail_or_test = 'vail'
    try:
        for data in ['train', vail_or_test]:
    ...
    print("\nTotal train images: %s" % (len(img_data['train'])),
          "\nTotal vail images: %s" % (len(img_data[vail_or_test])),
          "\n%.2f sec per 1000 image" % ((time.time() - since) * 1000 / (len(img_data['train']) + len(img_data[vail_or_test]))))
    ...
    hist = parallel_model.fit(img_data['train'], label_data['train'], batch_size=32*num_gpu, epochs=1, verbose=1,
                              validation_data=(img_data[vail_or_test], label_data[vail_or_test]), callbacks=[tensorboard])
