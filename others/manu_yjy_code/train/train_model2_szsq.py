...
num_gpu = 1
learning_rate = 1e-5
path_checkpoints = r'H:\weights\w_sdpc\model2\stage_new_class_szsq'
path_xlsx = {'train':path_checkpoints + '/2_train_37_3d1_3d2_our_szsq.xlsx',
             'test':path_checkpoints + '/2_test_37_3d1_3d2_our_szsq.xlsx'}

file_pretrain_weights = r'H:\weights\w_sdpc\model2\stage_new_class_szsq\Block_1560_3d1_3d2_our_szsq.h5'
start = 1561
end = 4001
Frozen_layers=37
...
for block in range(start, end):
    ...
    hist = parallel_model.fit(img_data['train'], label_data['train'], batch_size=48*num_gpu, epochs=1, verbose=1,
    ...
    if block % 2 == 0:
        if num_gpu > 1:
            model.save_weights(os.path.join(path_checkpoints, 'Block_%s_3d1_3d2_our_szsq.h5' % block))
        else:
            parallel_model.save_weights(os.path.join(path_checkpoints, 'Block_%s_3d1_3d2_our_szsq.h5' % block))
